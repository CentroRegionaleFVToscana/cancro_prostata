...
#### D6_Table_2_fup_<nome_farmaco> ----


# authors: Sabrina Giometto

# v 0.1 15 Sep 2026 Creation of D6 for Table 2


print('CREATE D6_Table_2_fup_<nome_farmaco>')
...
for (j in drug_names) {
  ...
  D5 <- read.csv(paste0(thisdirinput, "D5_Table_2_", j, ".csv"))

  D5 <- as.data.table(D5)

  assign(paste0("D5_Table_2_",j), D5)
...
for (k in drug_names) {
  ...
  tab_nice <- copy(get(paste0("D5_Table_2_", k)))
  ...
  # row 3
  row_header_1 <- c(row_header_1, "Totale, n (%)")
  j <- j + 1
  ...
  j <- descriptive_N_perc(j, "lostfup_")

  # row 6
  row_header_1 <- c(row_header_1, "Eventi cardiovascolari, n (%)")
  j <- descriptive_N_perc(j, "CV_fup_")

  # row 7
  row_header_1 <- c(row_header_1, "Infarto miocardico, n (%)")
  j <- descriptive_N_perc(j, "infart_fup_")

  # row 8
  row_header_1 <- c(row_header_1, "Cardiopatia ischemica, n (%)")
  j <- descriptive_N_perc(j, "cardioisc_fup_")
  
  # row 9
  row_header_1 <- c(row_header_1, "Ictus ischemico o emorragico, n (%)")
  j <- descriptive_N_perc(j, "ictus_fup_")
  
  # row 10
  row_header_1 <- c(row_header_1, "Scompenso cardiaco, n (%)")
  j <- descriptive_N_perc(j, "scompcard_fup_")
  
  # row 11
  row_header_1 <- c(row_header_1, "Angina, n (%)")
  j <- descriptive_N_perc(j, "angi_fup_")
  
  # row 12
  row_header_1 <- c(row_header_1, "Aritmie, n (%)")
  j <- descriptive_N_perc(j, "arit_fup_")
  
  # row 13
  row_header_1 <- c(row_header_1, "Insufficienza renale acuta, n (%)")
  j <- descriptive_N_perc(j, "renal_fup_")
  
  # row 14
  row_header_1 <- c(row_header_1, "Insufficienza epatica acuta, n (%)")
  j <- descriptive_N_perc(j, "epa_fup_")
  
  # row 15
  row_header_1 <- c(row_header_1, "Fratture, n (%)")
  j <- descriptive_N_perc(j, "fratt_fup_")
  ...
  setorder(tab_nice, rownum, user_type)
  ...
  first_cols   <- grep("^first", data_cols, value = TRUE)
  other_cols <- setdiff(data_cols, first_cols)
  setcolorder(tab_nice, c("row_header", first_cols, other_cols))
  ...
  newnames <- c(
    first = "Prima linea",
    nofirst = "Nuovi utilizzatori non prima linea"
  )
